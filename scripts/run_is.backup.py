...
try:
    from hdt.core.amu import amuize  # canonical
except Exception:
    try:
        from hdt.core.amu.extract import extract_amus as amuize
    except Exception:
...
try:
    ...
    def _ensure_file(target: Path, src: Path) -> bool:
        try:
            ...
            try:
                if getattr(target, "is_symlink", lambda: False)():
                    try: target.unlink()
                    except Exception: pass
                if target.exists():
                    try: target.unlink()
